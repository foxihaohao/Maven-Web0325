...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util','laydate'], function(exports){
  ...
  var $ = layui.$
      ...
      //字符常量
      ,MOD_NAME = 'tableFilter', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
      ...
      ,ELEM_CARD = '.layui-card',ELEM_CARD_BODY='.layui-card-body',ELEM_CARD_SEARCH='.layui-search',ELEM_CARD_TITLE='.layui-card-header'
      //thead区域模板
      ...
      //主模板
      ,TPL_MAIN = [
        '<div class="{{d.VIEW_CLASS}}">'
        ,'<form class="layui-form" style="margin:0 auto;margin-top: 10px;width: 99%;"><div class="layui-search layui-form-pane" style="border:1px solid #e6e6e6;border-radius:5px"></div></form>'
        ,'<div class="layui-form layui-border-box layui-table-view " lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
        ,'{{# if(d.data.toolbar){ }}'
        ...
        ,'</style>'
        ,'</div></div>'].join('')
      ,WRAP_CARD = ['<div class="layui-card" >',
        '<div class="layui-card-body"></div>',
      ].join('')
      ,WRAP_CARD_TITLE='<div class="layui-card-header"><blod>表格标题</blod></div>'
      ,WRAP_CARD_TITLE_TEXT='表格标题'
      ,WRAP_CARD_SEARCH_BUTTON=[
        '<div class="layui-inline">',
        '<button type="button" lay-submit="" lay-filter="#FILTER#" class="layui-btn layui-btn-primary layui-search-btn"><i class="layui-icon layui-icon-search"></i></button>',
        '</div>'
      ].join('')
      ,WRAP_CARD_SEARCH_BTN = '.layui-search-btn'
      ,WRAP_CARD_SEARCH='<div class="layui-search layui-form-pane"  style="border:1px solid #e6e6e6;border-radius:5px"></div>'
      ,_WIN = $(window)
      ,_DOC = $(document)
  ...
  Class.prototype.render = function(){
    ...
    //生成替代元素,原系统中将会把已经渲染好的元素重新删除掉]
    hasRender[0]&& hasRender.remove(); //如果已经渲染，则Rerender\
    othis.after(reElem);
    //附加容器
    that.timeGroup={};//时间存储
    that.layCard = reElem.find(ELEM_CARD);
    that.layCardBody = reElem.find(ELEM_CARD_BODY);
    that.layCardSearch=reElem.find(ELEM_CARD_SEARCH);
    that.layCardTitle = reElem.find(ELEM_CARD_TITLE);
    //各级容器
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    var that = this
        ...
        ,ex = res[options.response.exName]|| {}
        ,trs = []
        ,trs_fixed = []
        ,trs_fixed_r = []
        ,WRAP_CARD_SEARCH_ITEM_KEY='#KEY#'
        ,WRAP_CARD_SEARCH_ITEM_NAME='#NAME#'
        ,WRAP_CARD_SEARCH_ITEM_CONTENT='#CONTENT#'
        ,WRAP_CARD_SEARCH_DROP_ITME_SELECT='#SELECT#'
        ,WRAP_CARD_SEARCH_BTN_FILTER = '#FILTER#'
        ,WRAP_CARD_SEARCH_SWITCH_ITME='#TEXT#'
        ,WRAP_CARD_SEARCH_INPUT_LENGTH='#MAXLENGTH#'
        ,WRAP_CARD_SEARCH_INPUT_WIDTH='#NUM#'
        ,WRAP_CARD_SEARCH_BOX='<div class="layui-form-item" style="margin: 0 auto;margin-top: 5px;width: 99%;">#CONTENT#</div>'
        ,WRAP_CARD_SEARCH_DROP_ITME_DEFAULT='<option value="">请选择#NAME#</option>'
        ,WRAP_CARD_SEARCH_DROP_ITME='<option value="#KEY#">#NAME#</option>'
        ,WRAP_CARD_SEARCH_ITEM_MAP={//搜索映射
          text:{
            item:[
              '<div class="layui-inline" style="margin-right: 5px;margin-bottom:5px;width:#NUM#%;" >',
              '<label class="layui-form-label">#NAME#</label>',
              '<div class="layui-input-block">',
              '<input type="text" name="#KEY#" autocomplete="off"  maxlength="#MAXLENGTH#"  placeholder="请输入#NAME#" class="layui-input">',
              '</div></div>',
            ].join('')
          },
          date:{
            item:[   '<div class="layui-inline" style="margin-right: 5px;margin-bottom:5px;width:#NUM#%;" >',
              '<label class="layui-form-label">#NAME#</label>',
              '<div class="layui-input-block">',
              '<input type="text" name="#KEY#" id="#KEY#" autocomplete="off"  placeholder="请输入#NAME#" class="layui-input">',
              '</div></div>',
            ].join('')
          },
          drop:{
            item:[   '<div class="layui-inline" style="margin-right: 5px;margin-bottom:5px;width:#NUM#%;" >',
              '<label class="layui-form-label">#NAME#</label>',
              '<div class="layui-input-block">',
              '<select name="#KEY#" #SELECT# >#CONTENT#</select>',
              '</div></div>',
            ].join('')
          },
          switch:{
            item:[   '<div class="layui-inline" style="margin-right: 5px;margin-bottom:5px;width:#NUM#%;" >',
              '<label class="layui-form-label">#NAME#</label>',
              '<div class="layui-input-block" style="line-height: 35px;text-align: center;border: 1px solid #e6e6e6;height: 1px;border-radius: 0 2px 2px 0;border-left: none;">',
              '<input type="checkbox" checked="" name="#KEY#" lay-skin="switch"  lay-text="#TEXT#">',
              '</div></div>',
            ].join('')
          }
          ,
          build:function(content,data){
            var t = data.type;
            if("date"===t){
              if(data.group){
                var group = that.timeGroup[data.group];
                if(!(group instanceof Array)){
                  group=[];
                }
                if("start"===data.limit){
                  group.unshift(data.key);
                }else{
                  group.push(data.key);
                }
                that.timeGroup[data.group]=group;
              }
              return content.replace(WRAP_CARD_SEARCH_ITEM_KEY,data.key).replace(WRAP_CARD_SEARCH_ITEM_KEY,data.key).replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name).replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name);
            }else if("drop"===t){
              var objs = data.data;
              var s=[WRAP_CARD_SEARCH_DROP_ITME_DEFAULT.replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name)];
              for(var o in objs){
                s.push(WRAP_CARD_SEARCH_DROP_ITME.replace(WRAP_CARD_SEARCH_ITEM_KEY,o).replace(WRAP_CARD_SEARCH_ITEM_NAME,objs[o]));
              }
              var selectItem = data.select?'lay-search=""':'';
              return content.replace(WRAP_CARD_SEARCH_DROP_ITME_SELECT,selectItem).replace(WRAP_CARD_SEARCH_ITEM_KEY,data.key).replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name).replace(WRAP_CARD_SEARCH_ITEM_CONTENT,s.join(''));
            }else if("switch"===t){
              var txt = [data.yes.txt,'|',data.no.txt].join('');
              return content.replace(WRAP_CARD_SEARCH_ITEM_KEY,data.key).replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name).replace(WRAP_CARD_SEARCH_SWITCH_ITME,txt);
            }
            return content.replace(WRAP_CARD_SEARCH_ITEM_KEY,data.key).replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name).replace(WRAP_CARD_SEARCH_ITEM_NAME,data.name).replace(WRAP_CARD_SEARCH_INPUT_LENGTH,data.length);
          }
        }
        //渲染视图
        ...
        exRender = function(ex){//渲染搜索框的方法

          var filter = ex.filter||[],//[]
              result = '',
              colFilter = (options.colFilter?options.colFilter:5)-1,//默认是五行
              filterCount = 1,
              filters=[],
              colWidth=(100/colFilter)-0.4;
          if(filter.length==0){ that.layCardSearch.remove();return "";}
          $.each(filter,function(index,item){
            if(index%colFilter==colFilter){
              result+=WRAP_CARD_SEARCH_ITEM_MAP.build(WRAP_CARD_SEARCH_ITEM_MAP[item.type].item||'',item)
                  .replace(WRAP_CARD_SEARCH_INPUT_WIDTH,colWidth);
              filters.push(WRAP_CARD_SEARCH_BOX.replace(WRAP_CARD_SEARCH_ITEM_CONTENT,result));
              result='';
              filterCount++;
            }else{
              result+=WRAP_CARD_SEARCH_ITEM_MAP.build(WRAP_CARD_SEARCH_ITEM_MAP[item.type].item||'',item)
          ...
          result+=WRAP_CARD_SEARCH_BUTTON.replace(WRAP_CARD_SEARCH_BTN_FILTER,options.id);
          filters.push(WRAP_CARD_SEARCH_BOX.replace(WRAP_CARD_SEARCH_ITEM_CONTENT,result));

          // return WRAP_CARD_SEARCH_BOX.replace(WRAP_CARD_SEARCH_ITEM_CONTENT,result);
          return filters.join('');
        },
        exEvent = function(obj,ex){//事件注册
          //找到所有的条件输入框进行绑定
          var filter = ex.filter||[],//[]
              swit = {};//switch的目标
          $.each(filter,function(index,item){
            ...
            var t = item.type;
            if('date'===t){
              var group = that.timeGroup[item.group];
              //不存在说明只是单纯想要个日期输入框
          ...
          //查找到对应的搜索按钮进行绑定
          //监听提交
          form.on('submit('+options.id+')', function(data){
            //对switch进行特殊的处理,如果对应的内容没有被选择，则说明字段没有被选中，为false
            var d = data.field;
            for(var s in swit){
              if(d[s]){//存在
                d[s]=swit[s].yes.key;
    ...
    if(!$.isEmptyObject(ex)){
      //标题的内容
      // var title = ex.title,
      var cardTitle = that.layCardTitle,
          cardSearch = that.layCardSearch;
      cardSearch.html(exRender(ex));
      // cardTitle.html('<b>'+title+'</b>');
      exEvent(cardSearch,ex);
      form.render();
    }else{
      that.layCardSearch.remove();
    }
